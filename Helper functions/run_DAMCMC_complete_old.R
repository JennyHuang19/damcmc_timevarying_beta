...
#' (accounting for iota_infectious)
#'
...
run_DAMCMC_complete_old <- function(
  theta, Y, N = 10000,
  ...
  h_hat = c(0.30,0.90,0.30,0.90,0.30),
  x_b_ratio = 5,
  gamma=0.1
) {
  ...
  run_time <- system.time({
    ...
    # 1. Propose dbeta
    dbeta_current <- Rlab::rbern(length(h_hat), h_hat)
    u_current <- 1 # (CHANGE)
    ...
    for(s in 2 : N) {
      ...
      # 1. Propose dbeta
      dbeta_new <- draw_dbeta(dbeta_current, is_prob=TRUE, h_hat)[[1]]
      u_new <- draw_dbeta(dbeta_current, is_prob=TRUE, h_hat)[[2]]
      ...
      # Proposal density
      b_prop_current <- f_dbeta_func(dbeta_current, h_hat, u_current) + f_beta_func(unique_beta_current, par_prior, segmented_SS_current)
      b_prop_new <- f_dbeta_func(dbeta_new, h_hat, u_new) + f_beta_func(unique_beta_new, par_prior, segmented_SS_new)
      ...
      if(is.na(b_accept[s]) ){
        print(list("b_target_new" = b_target_new, "b_prior_new" = b_prior_new, "b_prop_new" = b_prop_new, "f_beta_func" = f_beta_func(unique_beta_new, par_prior, segmented_SS_new), "prior_beta" = prior_beta(beta_new, par_prior)))
      }
      ###
