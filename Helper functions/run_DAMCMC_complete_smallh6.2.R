#' This function is nearly identical to run_DAMCMC_complete_smallh6, except it takes in a known *fixed* sequence of change point locations,
#' rather than inferring the sequence via MCMC.
#' In the manuscript, this function is used to compare our proposed model to the time-homogeneous model, fixed change point model,
#' and PELT/Binseg model. (see figures 2 and 4 in the manuscript.)
#'
...
#' @param length_delta number of time segments specified.
#' @param x_b_ratio number of time that we update x for every update of beta.
#' @param gamma recovery rate, which we assume is known and fixed.
#'
