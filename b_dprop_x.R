#' Log proposal density for the latent data for time-varying beta (change-point version: keeps track of values of beta at each observation time T_k).
#'
...
b_dprop_x <- function(beta_vector,
  ...
  for(k in 1 : K){
    wk = floor(ts[k+1]) + 1 # compute the week in which we observe T_k[k] (+1 for indexing of vectors in R.)
    if(k == K){
      wk = floor(ts[k+1]) # adjustment for the last ts value. ([k+1] adjusts ts to line up with T_k)
    }
