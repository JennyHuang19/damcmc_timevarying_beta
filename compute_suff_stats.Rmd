...
#'
#' @return a list sufficient statistics, in which each element is the SS for a segment of beta.
#' @export
#'
...
suff_stat2 <- function(segment_start, segment_end, x, Y, gener, b, return_SI = FALSE) {
    ...
    for(i in 1:length(segment_start)){
      ...
      iota_infectious_segment <- segment_end[i]     - tau_T[infectious_after_segment]
      
      # wait times should be positive.
      if(any(iota_removed_segment < 1e-12))  return(list(compatible = FALSE))
      if(any(iota_infectious_segment < 1e-12))  return(list(compatible = FALSE))
      ...
      # the number of I(tau), S(tau) at each t.
      ...
      if(any(I_tau_T_segment == 0))  return(list(compatible = FALSE)) # depletion of infectious particles
...
gener = FALSE
b = 0.5
K=10 # observation time points
x <- SIR$x
Y     <- observed_data(SIR, K)

sufficient_statistics_epidemic <- suff_stat2(segment_start, segment_end, x, Y, gener, b, return_SI = FALSE)
sufficient_statistics_epidemic
```
