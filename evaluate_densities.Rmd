---
title: "evaluate densities"
author: "Jenny Yijian Huang"
date: "11/3/2021"
output: html_document
---
Helper functions to evaluate prior and proposal densities in the calculation of the acceptance ratio.

### A. Evaluate the prior densities.
...
# sum(unique(dgamma(beta_star, par_prior['a_beta'][[1]], par_prior['b_beta'][[1]], log = TRUE)))
```

```{r}
beta_star <- c(0.0025,0.0025,0.0015)
par_prior = list(
...
# P(dbeta_star | dbeta)
```{r}
#' LOG PROPOSAL DENSITY OF DELTA-BETA (DRAWN FROM A UNIFORM DISTRIBUTION THEN DRAWN FROM A BERNOULLI(H_HAT))
#'
#' @param dbeta delta beta vector (current)
...
f_dbeta_func <- function(dbeta_j, h_hat, u) {
  if(dbeta_j[u] == 1){
    prob_dbeta <- 1/length(dbeta_j) * (h_hat[u])
  }
  else{
    prob_dbeta <- 1/length(dbeta_j) * (1 - h_hat[u])
  }
