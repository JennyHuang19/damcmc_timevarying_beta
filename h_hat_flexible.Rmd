...
# change points in beta on weeks 2,4
```{r}
change_point_data2 <- simulate_change_point_SEM(
...
```{r}
plot(x = my_x$t, y = my_x$I) #no cp
plot(x = change_point_data2$t, y = change_point_data2$I) #cp
```


```{r}
sample_run_cp <- run_DAMCMC_beta(
  cp_data2, Y, N = 10000,
  ...
  h_hat = c(0.01,0.01,0.01,0.01,0.01) # P(dbeta_i = 0)
)
```

```{r}
sample_run_cp2$theta_rate_accept
```

```{r}
sample_run_cp$acc_dbeta_lst[90:100]
sample_run_cp$acc_beta_lst[90:100]
```

```{r}
# Change output into a df
b_lst <- sample_run_cp$acc_beta_lst
...
# R0 = c(2.5, 1.5, 3.5), change_day = c(2,4)
```{r}
