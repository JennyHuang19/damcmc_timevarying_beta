...
run_DAMCMC_complete_latent <- function(
  theta, Y, N = 10000,
  ...
  gamma=1
    ...
    # Setup
    f_save <- numeric(x_b_ratio*N / thin)
    b_f_save <- numeric(N / thin)
    accept <- numeric(x_b_ratio*N) # the proposals for x will happen "x_b_ratio" times as often as the proposals for theta.
    b_accept <- numeric(N)
    ...
    u_current <- 1 # (CHANGE) proposal density for the first draw?
    ...
    ### Print the segmented_SS for the last segment.
    print(list("n_T" = segmented_SS_current[[length(segmented_SS_current)]]$n_T, "integral_SI" = segmented_SS_current[[length(segmented_SS_current)]]$integral_SI, "n_T/integral_SI" =  segmented_SS_current[[length(segmented_SS_current)]]$n_T / segmented_SS_current[[length(segmented_SS_current)]]$integral_SI))
    ###
    ...
    for(s in 2 : N) {
      ...
      for(k in 1 : x_b_ratio){ # propose latent data "x_b_ratio" times as many times as theta.
          # 1. propose x_new
          ...
          R         <- min(1, exp(R_log))
          accept[x_b_ratio*(s-1)+k] <- stats::runif(1) < R # how do i keep track of the new indexing for the accept vector?
      
          # Accept/reject new draws
          if(accept[x_b_ratio*(s-1)+k]) {
            x_current        <- x_new
            SS_current       <- SS_new
...
run_DAMCMC_complete_2 <- function(
  theta, Y, N = 10000,
  rho = 0.5, param = "bg", approx = "ldp",
  iota_dist = "exponential", gener = FALSE, b = 1/2,
  thin = 100, plt = 1000,
